...
- (void)initialization
{
    self.dotViewClass           = [RPKAnimatedDotView class];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touch.view != self) {
        ...
        if ([self.delegate respondsToSelector:@selector(RPKPageControl:didSelectPageAtIndex:)]) {
            [self.delegate RPKPageControl:self didSelectPageAtIndex:index];
        }
    }
}
...
- (UIView *)generateDotView
{
    ...
    if (self.dotViewClass) {
        ...
        if ([dotView isKindOfClass:[RPKAnimatedDotView class]] && self.dotColor) {
            ((RPKAnimatedDotView *)dotView).dotColor = self.dotColor;
        }
    } else {
...
{
    if (self.dotViewClass) {
        RPKAbstractDotView *abstractDotView = (RPKAbstractDotView *)[self.dots objectAtIndex:index];
        if ([abstractDotView respondsToSelector:@selector(changeActivityState:)]) {
            [abstractDotView changeActivityState:active];
        } else {
            NSLog(@"Custom view : %@ must implement an 'changeActivityState' method or you can subclass %@ to help you.", self.dotViewClass, [RPKAbstractDotView class]);
        }
